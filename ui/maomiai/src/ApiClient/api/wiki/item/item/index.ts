/* tslint:disable */
/* eslint-disable */
// Generated by Microsoft Kiota
// @ts-ignore
import { Canal_document_tasksRequestBuilderRequestsMetadata, type Canal_document_tasksRequestBuilder } from './canal_document_tasks/index.js';
// @ts-ignore
import { Complate_uploadRequestBuilderRequestsMetadata, type Complate_uploadRequestBuilder } from './complate_upload/index.js';
// @ts-ignore
import { ConfigRequestBuilderRequestsMetadata, type ConfigRequestBuilder } from './config/index.js';
// @ts-ignore
import { Delete_documentRequestBuilderRequestsMetadata, type Delete_documentRequestBuilder } from './delete_document/index.js';
// @ts-ignore
import { DeleteRequestBuilderRequestsMetadata, type DeleteRequestBuilder } from './delete/index.js';
// @ts-ignore
import { DetailRequestBuilderRequestsMetadata, type DetailRequestBuilder } from './detail/index.js';
// @ts-ignore
import { Document_tasksRequestBuilderRequestsMetadata, type Document_tasksRequestBuilder } from './document_tasks/index.js';
// @ts-ignore
import { DocumentRequestBuilderNavigationMetadata, DocumentRequestBuilderRequestsMetadata, type DocumentRequestBuilder } from './document/index.js';
// @ts-ignore
import { DocumentsRequestBuilderRequestsMetadata, type DocumentsRequestBuilder } from './documents/index.js';
// @ts-ignore
import { PreuploadRequestBuilderRequestsMetadata, type PreuploadRequestBuilder } from './preupload/index.js';
// @ts-ignore
import { SimpleRequestBuilderRequestsMetadata, type SimpleRequestBuilder } from './simple/index.js';
// @ts-ignore
import { type UpdateRequestBuilder, UpdateRequestBuilderRequestsMetadata } from './update/index.js';
// @ts-ignore
import { type Upload_avatarRequestBuilder, Upload_avatarRequestBuilderRequestsMetadata } from './upload_avatar/index.js';
// @ts-ignore
import { type BaseRequestBuilder, type KeysToExcludeForNavigationMetadata, type NavigationMetadata } from '@microsoft/kiota-abstractions';

/**
 * Builds and executes requests for operations under /api/wiki/{teamId}/{wikiId}
 */
export interface WithWikiItemRequestBuilder extends BaseRequestBuilder<WithWikiItemRequestBuilder> {
    /**
     * The canal_document_tasks property
     */
    get canal_document_tasks(): Canal_document_tasksRequestBuilder;
    /**
     * The complate_upload property
     */
    get complate_upload(): Complate_uploadRequestBuilder;
    /**
     * The config property
     */
    get config(): ConfigRequestBuilder;
    /**
     * The delete_document property
     */
    get delete_document(): Delete_documentRequestBuilder;
    /**
     * The deletePath property
     */
    get deletePath(): DeleteRequestBuilder;
    /**
     * The detail property
     */
    get detail(): DetailRequestBuilder;
    /**
     * The document property
     */
    get document(): DocumentRequestBuilder;
    /**
     * The document_tasks property
     */
    get document_tasks(): Document_tasksRequestBuilder;
    /**
     * The documents property
     */
    get documents(): DocumentsRequestBuilder;
    /**
     * The preupload property
     */
    get preupload(): PreuploadRequestBuilder;
    /**
     * The simple property
     */
    get simple(): SimpleRequestBuilder;
    /**
     * The update property
     */
    get update(): UpdateRequestBuilder;
    /**
     * The upload_avatar property
     */
    get upload_avatar(): Upload_avatarRequestBuilder;
}
/**
 * Uri template for the request builder.
 */
export const WithWikiItemRequestBuilderUriTemplate = "{+baseurl}/api/wiki/{teamId}/{wikiId}";
/**
 * Metadata for all the navigation properties in the request builder.
 */
export const WithWikiItemRequestBuilderNavigationMetadata: Record<Exclude<keyof WithWikiItemRequestBuilder, KeysToExcludeForNavigationMetadata>, NavigationMetadata> = {
    canal_document_tasks: {
        requestsMetadata: Canal_document_tasksRequestBuilderRequestsMetadata,
    },
    complate_upload: {
        requestsMetadata: Complate_uploadRequestBuilderRequestsMetadata,
    },
    config: {
        requestsMetadata: ConfigRequestBuilderRequestsMetadata,
    },
    deletePath: {
        requestsMetadata: DeleteRequestBuilderRequestsMetadata,
    },
    delete_document: {
        requestsMetadata: Delete_documentRequestBuilderRequestsMetadata,
    },
    detail: {
        requestsMetadata: DetailRequestBuilderRequestsMetadata,
    },
    document: {
        requestsMetadata: DocumentRequestBuilderRequestsMetadata,
        navigationMetadata: DocumentRequestBuilderNavigationMetadata,
    },
    documents: {
        requestsMetadata: DocumentsRequestBuilderRequestsMetadata,
    },
    document_tasks: {
        requestsMetadata: Document_tasksRequestBuilderRequestsMetadata,
    },
    preupload: {
        requestsMetadata: PreuploadRequestBuilderRequestsMetadata,
    },
    simple: {
        requestsMetadata: SimpleRequestBuilderRequestsMetadata,
    },
    update: {
        requestsMetadata: UpdateRequestBuilderRequestsMetadata,
    },
    upload_avatar: {
        requestsMetadata: Upload_avatarRequestBuilderRequestsMetadata,
    },
};
/* tslint:enable */
/* eslint-enable */
