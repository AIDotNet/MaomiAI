...
public class GetTeamsQueryHandler : IRequestHandler<GetTeamsQuery, PagedResult<TeamDto>>
{
    ...
    public async Task<PagedResult<TeamDto>> Handle(GetTeamsQuery request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (totalCount == 0)
            {
                _logger.LogInformation("查询团队列表结果为空，条件: Keyword={Keyword}, Status={Status}, Page={Page}, PageSize={PageSize}",
            ...
            _logger.LogInformation("查询团队列表成功，条件: Keyword={Keyword}, Status={Status}, Page={Page}, PageSize={PageSize}, 总数={TotalCount}, 返回数量={Count}",
