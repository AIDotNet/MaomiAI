...
public class GetTeamMembersQueryHandler : IRequestHandler<GetTeamMembersQuery, PagedResult<TeamMemberDto>>
{
    ...
    public async Task<PagedResult<TeamMemberDto>> Handle(GetTeamMembersQuery request, CancellationToken cancellationToken)
    {
        try
        {
            // 验证团队是否存在
            ...
            if (totalCount == 0)
            {
                _logger.LogInformation("查询团队 {TeamId} 的成员列表结果为空，条件: Keyword={Keyword}, IsAdmin={IsAdmin}, Page={Page}, PageSize={PageSize}",
            ...
            _logger.LogInformation("查询团队 {TeamId} 的成员列表成功，条件: Keyword={Keyword}, IsAdmin={IsAdmin}, Page={Page}, PageSize={PageSize}, 总数={TotalCount}, 返回数量={Count}",
                request.TeamId, request.Keyword, request.IsAdmin, request.Page, request.PageSize, totalCount, items.Count);
