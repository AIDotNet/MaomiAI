...
public class TransferTeamOwnershipCommandHandler : IRequestHandler<TransferTeamOwnershipCommand>
{
    ...
    public async Task Handle(TransferTeamOwnershipCommand request, CancellationToken cancellationToken)
    {
        ...
        // 验证当前操作人是否是团队所有者
        var currentOwner = await _dbContext.TeamMembers
            .FirstOrDefaultAsync(m => m.TeamId == request.TeamId && m.UserId == currentUserId && m.IsRoot, cancellationToken)
            ?? throw new InvalidOperationException("您不是该团队的所有者，无法转移所有权");

        // 验证新所有者是否是团队成员
        var newOwner = await _dbContext.TeamMembers
            .FirstOrDefaultAsync(m => m.TeamId == request.TeamId && m.UserId == request.NewOwnerUserId, cancellationToken)
