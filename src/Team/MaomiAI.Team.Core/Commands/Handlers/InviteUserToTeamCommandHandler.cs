    ...
    public class InviteUserToTeamCommandHandler : IRequestHandler<InviteUserToTeamCommand, int>
    {
        ...
        public async Task<int> Handle(InviteUserToTeamCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                // 检查用户是否已经是团队成员
                TeamMemberEntity? existingMember = await _dbContext.TeamMembers
                    .FirstOrDefaultAsync(m => m.TeamId == request.TeamId && m.UserId == request.UserId && !m.IsDeleted,
                        cancellationToken);
                ...
                // 创建新的团队成员
                TeamMemberEntity? teamMember = new()
                {
                ...
                _logger.LogInformation("成功邀请用户 {UserId} 加入团队 {TeamId}, 操作者: {OperatorId}, 是否为管理员: {IsAdmin}",
                    request.UserId, request.TeamId, currentUserId, request.IsAdmin);

                return teamMember.Id;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "邀请用户 {UserId} 加入团队 {TeamId} 失败: {Message}", request.UserId, request.TeamId,
                    ex.Message);
                throw;
            }
        }
