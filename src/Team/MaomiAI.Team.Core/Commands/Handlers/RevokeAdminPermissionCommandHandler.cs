    ...
    public class RevokeAdminPermissionCommandHandler : IRequestHandler<RevokeAdminPermissionCommand, bool>
    {
        ...
        public async Task<bool> Handle(RevokeAdminPermissionCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                // 验证目标用户是否是团队成员
                TeamMemberEntity? targetMember = await _dbContext.TeamMembers
                ...
                _logger.LogInformation("成功撤销用户 {UserId} 在团队 {TeamId} 的管理员权限, 操作者: {OperatorId}", request.UserId,
                    request.TeamId, currentUserId);

                return true;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "撤销用户 {UserId} 在团队 {TeamId} 的管理员权限失败: {Message}", request.UserId, request.TeamId,
                    ex.Message);
                throw;
            }
        }
