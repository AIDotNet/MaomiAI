// <copyright file="ChatProcessingCommandHandler.cs" company="MaomiAI">
...
using MaomiAI.Chat.Shared.Commands;
...
namespace MaomiAI.Chat.Core.Chat;

public class ChatProcessingCommandHandler : IStreamRequestHandler<ChatProcessingCommand, string>
{
    ...
    public ChatProcessingCommandHandler(DatabaseContext databaseContext, IRedisDatabase redisDatabase, IMediator mediator, ILoggerFactory loggerFactory, IHttpClientFactory httpClientFactory, CustomKernelMemoryBuilder customKernelMemoryBuilder, SystemOptions systemOptions)
    {
    ...
    public async IAsyncEnumerable<string> Handle(ChatProcessingCommand request, CancellationToken cancellationToken)
    {
        var chatEntity = await _databaseContext.UserChats.FirstOrDefaultAsync(x => x.Id == request.Id);
        ...
        var pluginIds = DBJsonHelper.FromJsonString<HashSet<Guid>>(chatEntity.PluginIds) ?? new HashSet<Guid>();
    ...
    private async Task<IReadOnlyCollection<KernelPlugin>> BuildPluginsAsync(UserChatEntity chatEntity, HashSet<Guid> pluginIds, CancellationToken cancellationToken)
    {
        List<KernelPlugin> functions = new();

        var pluginEntities = await _databaseContext.TeamPlugins
            .Where(x => x.TeamId == chatEntity.TeamId && pluginIds.Contains(x.Id))
    ...
    private async Task<KernelPlugin> GetOpenApiPluginsAsync(TeamPluginGroupEntity groupEntity, IReadOnlyCollection<TeamPluginEntity> pluginEntities)
    {
        // 后续抽到一个方法命令中
        ...
        var fileEntity = await _databaseContext.Files
            .Where(x => x.Id == groupEntity.OpenapiFileId)
            .FirstOrDefaultAsync();

        if (fileEntity == null)
        {
            throw new BusinessException("插件{0}已失效", groupEntity.Name) { StatusCode = 429 };
        }
        ...
        KernelPlugin plugin = OpenApiKernelPluginFactory.CreateFromOpenApi(
            ...
            executionParameters: new OpenApiFunctionExecutionParameters()
            {
                ...
                ServerUrlOverride = new Uri(groupEntity.Server),
                LoggerFactory = _loggerFactory,
                OperationSelectionPredicate = (operation) =>
                {
                    // 只使用指定的插件
                    return pluginEntities.Any(p => p.Name == operation.Id || p.Path == operation.Path);
                },
