...
namespace MaomiAI.User.Core.Commands.Handlers
{
    ...
    public class ToggleUserStatusCommandHandler : IRequestHandler<ToggleUserStatusCommand>
    {
        ...
        {
            // 查询所有需要更新状态的用户
            List<UserEntity>? users = await _dbContext.User
            ...
            // 记录找到的用户数量和未找到的用户ID
            List<Guid>? foundUserIds = users.Select(u => u.Id).ToList();
            List<Guid>? notFoundUserIds = request.UserIds.Except(foundUserIds).ToList();
            ...
            _logger.LogInformation("成功切换用户状态: 状态={Status}, 用户数量={Count}, UserIds={UserIds}", request.Status,
                users.Count, string.Join(", ", foundUserIds));
        }
    }
}